...
class Loss(object):
    ...
    def save_log(self, name="logs/loss"):
        ...
        ax.set_ylim(np.min(self._loss["loss"])-np.min(self._loss["loss"])/10.0, np.max(self._loss["loss"])+np.max(self._loss["loss"])/10.0)
        ax.grid()
    ...
    def train(self):
        ...
        with tqdm(range(self._epoch)) as pbar:
            for i, ch in enumerate(pbar): #train
                ...
                loss, _ = self._vae.train(self._sess, input_images)
